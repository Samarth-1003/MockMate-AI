...
const InnerApp = () => {
  ...
  // --- FIXED GO HOME FUNCTION ---
  const goHome = () => {
    // 1. Remove hash from URL (e.g. #about) to prevent scrolling down
    if (window.location.hash) {
        window.history.pushState("", document.title, window.location.pathname + window.location.search);
    }
    
    // 2. Hide the Form Interface (Show Hero Text)
    setIsStarted(false);
    
    // 3. Reset the Interview Step to 'setup' (Crucial for resetting the flow)
    setStep('setup');
  ...
  return (
    <div className="w-full h-screen overflow-y-auto overflow-x-hidden bg-[#0f172a] scroll-smooth relative">
      ...
      {/* Pass the fixed goHome function to Header */}
      ...
      <section id="home" className="relative w-full min-h-screen flex pt-20">
          ...
          {/* 1. HERO TEXT (Visible only when NOT started) */}
